 ...
 */ class Game {
   ...
   */ constructor(initialState){
    ...
    moveLeft() {
        ...
        for(let key = 0; key < state.length; key++){
            ...
            for(let num = 0; num < state[key].length; num++)if (state[key][num] === state[key][num + 1]) {
                state[key][num] *= 2;
                state[key][num + 1] = 0;
                this.score += +state[key][num];
                state[key] = removeZeros(state[key]);
            }
            for(let i = state[key].length; i < keyLength; i++)state[key].push(0);
        }
    ...
    moveRight() {
        ...
        for(let key = 0; key < state.length; key++){
            ...
            for(let i = state[key].length; i < keyLength; i++)state[key].unshift(0);
        }
        this.checkChangesState(oldState);
        this.updateUI();
    }
   ...
   */ getStatus() {
        if (this.status === "idle") {
            ...
            this.status = "playing";
        } else if (this.status === "win") document.querySelector(".message-win").classList.remove("hidden");
    }
    /**
   * Starts the game.
   */ start() {
   ...
   */ restart() {
        ...
        this.score = 0;
        if (this.status === "win") this.status = "playing";
        this.getStatus();
        this.getScore();
        this.updateUI();
    }
    ...
    addRandomTile() {
        ...
        function getRandom(isNumber = false) {
            ...
            if (isNumber && statusCopy === "playing") {
                ...
                if (randomNumber < 0.9) return 2;
                else return 4;
            } else if (isNumber && statusCopy === "idle") {
                randomNumber = Math.floor(Math.random() * 2);
                if (randomNumber === 0) return 2;
                else return 4;
    ...
    checkChangesState(oldStateArr) {
        ...
        if (oldArr !== newArr) this.addRandomTile(true);
            ...
            for(let i = 0; i < grid.length; i++)for(let j = 0; j < grid.length; j++){
                ...
                if (j + 1 < grid.length && current === grid[i][j + 1] || i + 1 < grid.length && current === grid[i + 1][j]) isTrue = true;
            }
            if (!isTrue) {
                this.status = "lose";
                this.getStatus();
    ...
    updateUI() {
        ...
        for(let key = 0; key < allCells.length; key++)if (this.state.flat()[key]) {
            allCells[key].textContent = this.state.flat()[key];
            allCells[key].classList = `field-cell field-cell--${allCells[key].textContent}`;
        } else {
